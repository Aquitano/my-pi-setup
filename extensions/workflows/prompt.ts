...
export const WORKFLOW_TOOL_DESCRIPTION = [
  ...
  "Workflow JavaScript runs in an isolated QuickJS WebAssembly heap inside a restricted, killable child with no imports, timers, filesystem, network, or process APIs. Each execution slice (including microtasks) has a one-second limit and the interpreter has a 128 MiB memory limit. A run may make at most 32 agent calls and 256 phase updates, and has no overall deadline. Each agent must receive its first assistant response event within 45 seconds so silent provider requests fail clearly; after that, agent() has no wall-clock deadline. Each individual child tool call times out independently after 3 minutes, becomes an error tool result, and leaves the agent loop free to recover. Use map/filter/if/await/template strings to orchestrate, and `return` a JSON-serializable aggregate.",
