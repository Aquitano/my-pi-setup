...
/** Describes subagent_spawn, including harnesses and the shared agent limit. */
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background subagent: a headless agent with its own context window and user-configured permissions. Claude and Codex default to automatic permission review; denied actions stay denied. You choose the harness it runs on: pi (in-process pi session, inherits this environment's tools and config), claude (Claude Code), or codex (Codex CLI). Fire-and-forget: this returns immediately with an id. The subagent's final output is queued back to you as a message when it settles, or collect it explicitly with subagent_wait. Children cannot orchestrate more agents/workflows or ask the user, and cannot see this conversation, so the prompt must be self-contained. Only use trusted working directories. Pass isolation: \"worktree\" when the child will edit files while other work touches the same repository: it then runs in its own git worktree on a fresh branch, and the result reports that branch for you to inspect or merge. Subagents and workflow agents share one running-agent limit (4 unless the user raised it), so spawns fail when it is full.";
